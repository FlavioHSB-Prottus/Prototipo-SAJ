document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderPessoaModal(data) {
        ...
        // Enderecos
        if (p && p.id) {
            var pnomEncEnd = encodeURIComponent(p.nome_completo || '');
            var pessoaIdEnd = String(p.id);
            ...
            html += '<button type="button" class="action-btn btn-add-endereco-pessoa" data-pessoa-id="' + pessoaIdEnd + '" data-pessoa-nome="' + pnomEncEnd + '" data-pessoa-papel="pessoa"><i class="fa-solid fa-plus"></i> Endereco</button>';
            html += '</div>';
            if (data.enderecos && data.enderecos.length > 0) {
                data.enderecos.forEach(function (e) {
                    ...
                    var _fonteEndP = (typeof window.formatContatoFonteLabel === 'function') ? window.formatContatoFonteLabel(e.fonte) : '';
                    html += dataItem('Fonte', _fonteEndP || '-');
                    html += '</div>';
                });
            } else {
            ...
            data.enderecos.forEach(function (e) {
                ...
                html += dataItem('Cidade', e.cidade);
                html += dataItem('Estado', e.estado);
                var _fonteEndP2 = (typeof window.formatContatoFonteLabel === 'function') ? window.formatContatoFonteLabel(e.fonte) : '';
                html += dataItem('Fonte', _fonteEndP2 || '-');
                html += '</div>';
            });
            html += '</div>';
        }
    ...
    function renderContratoModal(data) {
        ...
        // Ocorrencias (timeline: mais recente primeiro; ver ContratoDetalhesModal)
