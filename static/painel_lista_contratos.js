/**
 * Lista de contratos alinhada ao painel (Dashboard / Performance) + busca estilo módulo Busca.
 * Requer: busca.css, tabela .styled-table, função onDetalhe(id) no parent (ex.: openDetails do dashboard).
 */
(function () {
    'use strict';
            ...
            rows.forEach(function (c) {
                ...
                if (mode === 'performance') {
                    extra =
                        ...
                        (c.desempenho && String(c.desempenho).indexOf('Não') >= 0 ? 'status-warning' : 'status-success') +
                        '">' +
                        esc(c.desempenho || '-') +
        ...
        if (tipoEl) {
            tipoEl.addEventListener('change', function () {
                if (this.value === 'pessoa' && termoEl) {
                    ...
                    termoEl.placeholder = 'Descrição do bem...';
                } else if (termoEl) {
