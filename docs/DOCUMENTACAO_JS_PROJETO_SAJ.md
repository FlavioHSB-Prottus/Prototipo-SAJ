...
**Exportacao** (`doExport`): exige `currentResponse` (dados ja carregados). POST **`/api/performance/export/<formato>`** com `formato` em `xlsx` | `pdf` | `powerbi`. Corpo JSON: `mes` (`lastMes`), `safra_index` (`'all'` ou indice numerico), `series` fixo `['novos','pagos','indenizados']`, `faixas` derivadas do teto (`faixasForExportFromAtrasoTeto`: so `d30` se teto 30; `d30`+`d60` se 60; as tres se 90), `atraso_teto`. **PDF:** acrescenta `bar_image` (PNG base64 do grafico de barras via Chart.js) e `pie_image` vazio. Resposta: `blob` + download pelo `Content-Disposition`. O **.xlsx** inclui resumo por safra (quantidade + R$ parcela; performado cumulativo na aba larga; sem R$ credito no resumo) e aba pivot; ver `DOCUMENTACAO_PYTHON_PROJETO_SAJ.md`, secao 3.4.1.
